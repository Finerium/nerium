...
7. **No non-Anthropic model in the shipped reasoning layer.** Worker runtime, Apollo Advisor reasoning, Cassandra Prediction simulation, Heracles Managed Agents lane, Proteus translation, and every other Claude Code specialist must run on Opus 4.7 with the single Sonnet 4.6 exception for Cassandra. No Gemini, OpenAI, Llama, Higgsfield, or other vendor in the reasoning path of a shipped build. This preserves the "Built with Opus 4.7" characterization for the reasoning layer. Asset generation is scoped separately per the override at `docs/adr/ADR-override-antipattern-7.md`: CC0 packs (Kenney.nl, OpenGameArt, Oak Woods brullov, Warped City) plus Opus SVG and Canvas procedural remain primary; fal.ai Nano Banana 2 is authorized in principle for identity-critical sprites but ships DORMANT in the RV build per `_meta/RV_PLAN.md` RV.14 and Ghaisan personal fund USD 0 constraint. Reactivation of the fal.ai lane in a shipped build requires a superseding ADR.
